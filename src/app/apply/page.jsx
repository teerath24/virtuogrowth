...
function ApplyForm() {
  ...
  const [selectedRole, setSelectedRole] = useState(roleMap[roleParam] || "");
  const prevRoleParamRef = useRef(roleParam);

  useEffect(() => {
    if (prevRoleParamRef.current !== roleParam) {
      prevRoleParamRef.current = roleParam;
  ...
  //     developer: "web-developer",
  //   };
  //   if (roleMap[role]) setSelectedRole(roleMap[role]);
  // }, [searchParams]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        router.push("/thank-you?type=applicant");
      }, 3000);
    } catch (error) {
    ...
    <>
      ...
      <div>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
          ...
          shareable link here. Make sure it&apos;s set to &quot;Anyone with the
          link can view&quot;.
        </p>
      </div>
    </>
